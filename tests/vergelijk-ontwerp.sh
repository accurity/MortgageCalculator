...
#
#   tests/vergelijk-ontwerp.sh <pad-naar-ontwerp.html> [url]
#
# Het eerste argument is het uitgepakte artifact-bestand van het ontwerp
# (Hypotheekcalculator.dc.html uit Claude Design). De url wijst naar een
...
ONTWERP=${1:?geef het pad naar het ontwerp-html-bestand}
...
WERKMAP=$(mktemp -d)
trap 'rm -f "$WEBROOT/_harnas.html" "$(dirname "$ONTWERP")/_harnas.html"; rm -rf "$WERKMAP"' EXIT
...
vergelijk () { # naam, acties
  ...
  printf '%-24s ' "$1"
  python3 "$HIER/pixeldiff.py" "$WERKMAP/ontwerp.png" "$WERKMAP/site.png" \
    --negeer 1020,$((H-60)),$W,$H --uit "$WERKMAP/diff-$1.png" | head -1
}
